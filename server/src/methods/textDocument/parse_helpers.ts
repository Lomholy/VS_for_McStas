...
export function stripCommentsAndStrings(src: string): string {
  ...
  let i = 0;
  const N = src.length;
  enum Mode { Code, SLComment, MLComment, SQuote, DQuote }
  let mode = Mode.Code;
  while (i < N) {
    const c = src[i];
    const n = src[i + 1];
    ...
    if (mode === Mode.Code) {
      if (c === '/' && n === '/') { mode = Mode.SLComment; i += 2; continue; }
      if (c === '/' && n === '*') { mode = Mode.MLComment; i += 2; continue; }
      if (c === "'") { mode = Mode.SQuote; out += ' '; i++; continue; }
      if (c === '"') { mode = Mode.DQuote; out += ' '; i++; continue; }
      out += c; i++;
    } else if (mode === Mode.SLComment) {
      if (c === '\n') { mode = Mode.Code; out += '\n'; }
      i++;
    } else if (mode === Mode.MLComment) {
      if (c === '*' && n === '/') { mode = Mode.Code; i += 2; }
      else i++;
    } else if (mode === Mode.SQuote) {
      if (c === '\\') { i += 2; }        // skip escape
      else if (c === "'") { mode = Mode.Code; i++; }
      else i++;
...
export function splitTopLevelBy(s: string, delim: string): string[] {
  const parts: string[] = [];
...
export function removeTopLevelInitializer(s: string): string {
 ...
 */
export function looksLikeFunctionPrototype(stmt: string): boolean {
  ...
  for (let i = 0; i < s.length; i++) {
    const c = s[i];
    if (c === '(') {
      if (depth === 0) return true; // a top-level '('
      depth++;
    } else if (c === ')') {
 ...
 *   "char *names[3]" -> names
 */
export function extractIdentifierFromDeclarator(decl: string): string | null {
  let i = decl.length - 1;
  // Remove trailing whitespace
  while (i >= 0 && /\s/.test(decl[i])) i--;
  ...
  while (i >= 0 && (decl[i] === '*' || /\s/.test(decl[i]))) i--;

  // Now read the identifier backwards
  let end = i;
  while (i >= 0 && /[A-Za-z0-9_]/.test(decl[i])) i--;
  ...
  if (kw.has(ident)) {
    ...
    let j = i;
    while (j >= 0) {
      while (j >= 0 && !/[A-Za-z0-9_]/.test(decl[j])) j--;
      if (j < 0) break;
      let end2 = j;
      while (j >= 0 && /[A-Za-z0-9_]/.test(decl[j])) j--;
      const id2 = decl.slice(j + 1, end2 + 1);
      if (!kw.has(id2)) return id2;
    }
    return null;
  }
  return ident;
