...
type CondaEnvInfo = { name: string; path: string };
...
function getCondaExe():
  | { exe: string; kind: 'conda' | 'mamba' | 'micromamba' }
  | null {
  if (process.env.CONDA_EXE?.trim()) return { exe: process.env.CONDA_EXE.trim(), kind: 'conda' };
  if (process.env.MAMBA_EXE?.trim()) return { exe: process.env.MAMBA_EXE.trim(), kind: 'mamba' };
  if (process.env.MICROMAMBA_EXE?.trim()) return { exe: process.env.MICROMAMBA_EXE.trim(), kind: 'micromamba' };
  if (which('conda')) return { exe: 'conda', kind: 'conda' };
  if (which('mamba')) return { exe: 'mamba', kind: 'mamba' };
  if (which('micromamba')) return { exe: 'micromamba', kind: 'micromamba' };
  return null;
}

/** List environments with `conda env list --json` */
function listCondaEnvs(): CondaEnvInfo[] {
  const conda = getCondaExe();
  if (!conda) return [];

  const out = spawnSync(conda.exe, ['env', 'list', '--json'], { encoding: 'utf8' });
  if (out.status !== 0) return [];

  try {
    const parsed = JSON.parse(out.stdout) as { envs: string[]; default_prefix?: string };
...
function loadEnvPackageSet(envPath: string, condaExe: string): Set<string> | null {
  const out = spawnSync(condaExe, ['list', '--json', '-p', envPath], { encoding: 'utf8' });
  if (out.status !== 0) return null;

  try {
    const pkgs = JSON.parse(out.stdout) as Array<{ name: string }>;
...
function envHasAllPackages(envPath: string, required: string[], condaExe: string): boolean {
...
export function hasAnyEnvWithMcStasAndFlask(
  ...
  for (const env of listCondaEnvs()) {
    if (envHasAllPackages(env.path, required, conda.exe)) return true; // short‑circuit
  }
  return false;
}

/** Optional: check a specific env by prefix path directly */
export function specificEnvHasMcStasAndFlask(
  envPrefix: string,
  required: string[] = ['mcstas', 'flask']
): boolean {
  const conda = getCondaExe();
